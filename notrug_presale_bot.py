...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        ...
        # Step 0: Presale bilgisi
        if any(phrase in text for phrase in ["presale", "buy", "how", "price", "fiyat", "nasıl", "almak", "join"]):
            await presale_cmd(update, context)
            return

        # Step 1: TX hash bekleniyor
        if step == 0:
            # Kullanıcıdan TX hash iste
            context.user_data['tx_hash'] = text.strip()
            context.user_data['step'] = 1
            await update.message.reply_text(
                "✅ *TX hash received!*\n\n"
                "Now send your *Phantom wallet address* so we can send your NOTRUG! 👇",
                parse_mode="Markdown"
            )
            return

        # Step 2: Wallet adresi bekleniyor
        if step == 1:
            wallet = text.strip()
            ...
            admin_msg = (
                ...
                f"📊 Formula: SOL amount × {NOTRUG_PER_SOL:,} = NOTRUG\n\n"
                f"⚡ Check TX on solscan.io → Send NOTRUG manually!"
            )
