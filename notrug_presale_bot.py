...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("💰 Join Presale", callback_data="presale")],
        [InlineKeyboardButton("🌐 notrug.fun", url="https://notrug.fun")],
        [InlineKeyboardButton("✈️ Telegram Group", url="https://t.me/notrugfun")]
    ]
    text = (
        "*$NOTRUG Presale Bot* 🛡️\n\n"
        "Welcome! This is the official $NOTRUG presale bot.\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # DM handler - TX hash ve cüzdan adresi
    if chat_type == "private":
        # TX hash kontrolü (Solana TX hash 87-88 karakter)
        words = text.split()
        for word in words:
            if len(word) > 80 and len(word) < 100:
                # TX hash bulundu
                await update.message.reply_text(
                    "✅ TX hash received!\n\n"
                    "Now please send your *Solana wallet address* so we can send your NOTRUG!",
                    parse_mode="Markdown"
                )
                context.user_data['tx_hash'] = word
                return

        # Solana cüzdan adresi kontrolü (32-44 karakter)
        for word in words:
            if 32 <= len(word) <= 44 and word.isalnum():
                tx_hash = context.user_data.get('tx_hash', 'Unknown')
                sol_amount = context.user_data.get('sol_amount', 0)
                notrug_amount = sol_amount * NOTRUG_PER_SOL
                ...
                await update.message.reply_text(
                    ...
                    f"Your wallet: `{word[:10]}...`\n\n"
                    f"We will send your NOTRUG shortly! ✅\n\n"
                    f"_Usually within 1-2 hours_",
                    parse_mode="Markdown"
                )
                ...
                admin_msg = (
                    ...
                    f"🆔 ID: `{user.id}`\n"
                    f"💰 SOL sent: Check TX\n"
                    f"📊 NOTRUG to send: Check TX amount × {NOTRUG_PER_SOL:,}\n"
                    f"💼 Send to: `{word}`\n"
                    f"🔗 TX: `{tx_hash}`\n\n"
                    f"⚡ Action needed: Send NOTRUG manually!"
                )
        ...
        # Presale bilgisi iste
        if any(phrase in text for phrase in ["presale", "buy", "how", "price", "fiyat", "nasıl", "almak"]):
